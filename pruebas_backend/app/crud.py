...
def create_funcionalidades_proceso(db: Session, funcionalidades_proceso: schemas.FuncionalidadesProcesoCreate):
    db_funcionalidades_proceso = models.FuncionalidadesProceso(**funcionalidades_proceso.dict())
...
def get_funcionalidades_procesos(db: Session, skip: int = 0, limit: int = 10):
    return db.query(models.FuncionalidadesProceso).offset(skip).limit(limit).all()
...
def update_funcionalidades_proceso(db: Session, funcionalidades_proceso_id: int, funcionalidades_proceso: schemas.FuncionalidadesProcesoUpdate):
    ...
    db_funcionalidades_proceso.id_funcionalidad = funcionalidades_proceso.id_funcionalidad
    db_funcionalidades_proceso.id_proceso = funcionalidades_proceso.id_proceso
    db.commit()
    db.refresh(db_funcionalidades_proceso)
    return db_funcionalidades_proceso
...
def create_testers_proceso(db: Session, testers_proceso: schemas.TestersProcesoCreate):
    db_testers_proceso = models.TestersProceso(**testers_proceso.dict())
...
def get_testers_procesos(db: Session, skip: int = 0, limit: int = 10):
    return db.query(models.TestersProceso).offset(skip).limit(limit).all()
...
def create_tester(db: Session, tester: schemas.TesterCreate):
    db_tester = models.Tester(**tester.dict())
    db.add(db_tester)
    db.commit()
    db.refresh(db_tester)
    return db_tester

def get_tester(db: Session, tester_id: int):
    return db.query(models.Tester).filter(models.Tester.id == tester_id).first()

def get_testers(db: Session, skip: int = 0, limit: int = 10):
    return db.query(models.Tester).offset(skip).limit(limit).all()

def update_tester(db: Session, tester_id: int, tester: schemas.TesterUpdate):
    db_tester = db.query(models.Tester).filter(models.Tester.id == tester_id).first()
    db_tester.nombre = tester.nombre
    db.commit()
    db.refresh(db_tester)
    return db_tester

def delete_tester(db: Session, tester_id: int):
    db_tester = db.query(models.Tester).filter(models.Tester.id == tester_id).first()
    if db_tester:
        db.delete(db_tester)
        db.commit()
